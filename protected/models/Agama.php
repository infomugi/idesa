...
class Agama extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('kd_agama, mnemonic, nama', 'required'),
			array('kd_agama, nama', 'unique'),
			array('kd_agama', 'numerical', 'integerOnly'=>true),
			array('mnemonic', 'length', 'max'=>1),
			array('nama', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, kd_agama, mnemonic, nama', 'safe', 'on'=>'search'),
		);
	}
